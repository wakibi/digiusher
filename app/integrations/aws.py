...
from .constants import (
    AWS_BASE_URL,
    AWS_OFFER_CODES_TO_RUN,
    AWS_SERVICE_INDEX_FILE,
...
def persist_aws_data_using_csv(region_code, url):
    ...
    with closing(requests.get(url, stream=True)) as response:
        ...
        for counter, row in enumerate(aws_reader):
            ...
            clock_speed = get_float_or_null(row[24])
            ram = get_float_or_null(row[25])
            currency = row[10]
            unit = row[8]
...
def import_aws_data():
    ...
    region_index_urls = [
        offer["currentRegionIndexUrl"]
        for offer_code, offer in service_index_data["offers"].items()
        if offer_code in AWS_OFFER_CODES_TO_RUN
    ]
    ...
    for region_index_url in region_index_urls:
        ...
        for region_code, details in region_index_data["regions"].items():
            ...
            persist_aws_data_using_csv(
                region_code,
                service_price_list_url.format(base_url=AWS_BASE_URL, uri=current_region_index_url),
            )
